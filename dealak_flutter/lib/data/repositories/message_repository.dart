...
class MessageRepository {
  ...
  Future<PaginatedResponse<ConversationModel>> getConversations({int page = 1}) async {
    final response = await _dioClient.get(ApiEndpoints.conversations, queryParameters: {'page': page});
    return PaginatedResponse.fromJson(response.data, ConversationModel.fromJson);
  }

  Future<Map<String, dynamic>> createConversation(int recipientId, String message, {int? propertyId}) async {
    final response = await _dioClient.post(ApiEndpoints.conversations, data: {
  ...
  Future<PaginatedResponse<MessageModel>> getMessages(int conversationId, {int page = 1}) async {
  ...
  Future<MessageModel> sendMessage(int conversationId, String body, {String type = 'TEXT'}) async {
